...
import { cutoutGrid, cutoutSummary, cutoutFolderFilter, cropGrid, cropSummary, cropFolderFilter, viewModeSelect } from "./dom.js";
...
export function renderCutouts(cutouts, processing) {
  ...
  const visibleCutouts = cutouts.filter((item) => deps.matchesSelectedFolder(item.folder || "", selectedFolder));
  deps.pruneSelectionSet(selectedCutoutIds, cutouts.map((item) => item.id));
  ...
  if (viewModeSelect.value === "folders") {
    deps.renderFolderGroupedCollection(cutoutGrid, visibleCutouts.slice(0, 12), createCutoutCard, "Nenhum PNG sem fundo encontrado nesta visualização.");
  } else {
...
export function renderCrops(crops) {
  ...
  const visibleCrops = crops.filter((item) => deps.matchesSelectedFolder(item.folder || "", selectedFolder));
  deps.pruneSelectionSet(selectedCropIds, crops.map((item) => item.id));
  deps.updateBulkSelectionUi();
  ...
  if (viewModeSelect.value === "folders") {
    deps.renderFolderGroupedCollection(cropGrid, visibleCrops.slice(0, 12), createCropCard, "Nenhum crop encontrado nesta visualização.");
  } else {
