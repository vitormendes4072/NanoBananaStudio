...
function renderDailyCostChart(dailyCosts) {
  ...
  const bars = dailyCosts.map((d, i) => {
    const val = getValue(d);
    const barH = maxVal > 0 ? Math.max(2, (val / maxVal) * chartH) : 2;
    const x = PAD.left + i * barSpacing + (barSpacing - barW) / 2;
    const y = PAD.top + chartH - barH;
    const label = d.date.slice(5);
    const showLabel = dailyCosts.length <= 31 && (i === 0 || i === dailyCosts.length - 1 || (i % Math.ceil(dailyCosts.length / 8)) === 0);
    return [
      `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${barW.toFixed(1)}" height="${barH.toFixed(1)}" class="analytics-bar"/>`,
      showLabel ? `<text x="${(x + barW / 2).toFixed(1)}" y="${(PAD.top + chartH + 20).toFixed(1)}" class="analytics-tick analytics-tick-x">${escapeHtml(label)}</text>` : "",
    ].join("");
  }).join("\n");
