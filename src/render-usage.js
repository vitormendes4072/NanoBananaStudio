...
function formatMoney(value, currency) {
  return new Intl.NumberFormat("pt-BR", { style: "currency", currency: currency || "USD", minimumFractionDigits: 2, maximumFractionDigits: 4 }).format(value || 0);
}
...
export function renderUsage(data) {
  ...
  if (!data.byModel?.length) { usageModels.innerHTML = ""; }
  else {
    usageModels.innerHTML = data.byModel.map((entry) => `
      <article class="usage-model-card"><p class="usage-label">${escapeHtml(entry.label)}</p><p class="usage-value">${entry.completed} imagem(ns)</p><p class="usage-note">Estimado: ${formatMoney(entry.estimatedCost, entry.currency)} - ${formatMoney(entry.unitCost, entry.currency)}/imagem</p></article>`).join("");
  }
  ...
  for (const [label, url] of Object.entries(data.links || {})) {
    ...
    anchor.href = url; anchor.target = "_blank"; anchor.rel = "noreferrer"; anchor.textContent = linkLabel(label);
