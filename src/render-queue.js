...
function buildJobMeta(job) {
  ...
  if (job.status === 'processing')
    return [model, references, `iniciou ${formatDate(job.startedAt) || created}`]
      .filter(Boolean)
      .join(' - ');
  if (job.status === 'completed')
    return [model, references, `concluído ${formatDate(job.finishedAt) || created}`]
      .filter(Boolean)
      .join(' - ');
  if (job.status === 'failed')
    return [model, references, `falhou ${formatDate(job.finishedAt) || created}`]
      .filter(Boolean)
      .join(' - ');
...
function buildGalleryMeta(job) {
  return [modelLabel(job.model), referenceLabel(job.referenceImages), formatDate(job.finishedAt)]
    .filter(Boolean)
    .join(' - ');
}
...
function buildJobActions(job) {
  ...
  if (job.status === 'queued')
    return `<div class="queue-actions"><button class="queue-button" type="button" data-cancel-id="${job.id}">Cancelar</button></div>`;
  if (job.status === 'failed')
...
function galleryEmptyMessage(allCompletedJobs) {
  if (!allCompletedJobs.length)
    return 'Nenhuma imagem concluída ainda. Aguarde os jobs em processamento terminarem.';
