...
import {
  promptInput, promptAutocomplete, promptAutocompleteList, negativePromptInput,
  promptStrengthSelect, renderFocusSelect, aspectRatioSelect, styleDirectionInput,
  preserveDetailsInput, extraInstructionsInput, customPresetNameInput, customPresetList,
  statusBox,
} from "./dom.js";
import { CUSTOM_PRESETS_STORAGE_KEY, persistCustomPromptPresetsToStorage, sanitizePromptOptions } from "./prompt-presets-store.js";
...
export function resolvePromptProductModels(prompt) {
  ...
  const matchedModels = aliases.map((alias) => state.productModels.find((e) => e.alias === alias)).filter(Boolean);
  let cleanPrompt = String(prompt || "");
  for (const model of matchedModels) cleanPrompt = cleanPrompt.replace(new RegExp(`(^|\\s)@${model.alias}(?=\\s|$)`, "gi"), "$1");
  return { aliases: matchedModels.map((e) => e.alias), matchedModels, cleanPrompt: cleanPrompt.replace(/\s{2,}/g, " ").trim() };
}
...
export function resolvePromptImageTemplates(prompt) {
  ...
  const matchedTemplates = aliases.map((alias) => state.imageTemplates.find((e) => e.alias === alias)).filter(Boolean);
  let cleanPrompt = String(prompt || "");
  for (const template of matchedTemplates) cleanPrompt = cleanPrompt.replace(new RegExp(`(^|\\s)#${template.alias}(?=\\s|$)`, "gi"), "$1");
  return { aliases: matchedTemplates.map((e) => e.alias), matchedTemplates, cleanPrompt: cleanPrompt.replace(/\s{2,}/g, " ").trim() };
}
...
export function buildLocalizedPrompt(prompt, promptOptions = {}, regionReference) {
  ...
  for (const model of state.productModels) {
    sections.push(`Use o modelo de produto @${model.alias} como referência principal, mantendo fidelidade real ao produto ${model.name}.`);
  ...
  if (promptOptions.renderFocus) sections.push(`Foco principal: ${deps.humanizePromptFocus(promptOptions.renderFocus)}.`);
  if (promptOptions.aspectRatio) sections.push(`Use proporção de imagem ${promptOptions.aspectRatio}.`);
  if (promptOptions.promptStrength === "strong") sections.push("Siga o pedido com alta aderência, mantendo forte fidelidade aos detalhes e restrições descritos.");
  else if (promptOptions.promptStrength === "soft") sections.push("Interprete o pedido com mais liberdade criativa, preservando a intencao geral.");
  if (promptOptions.styleDirection) sections.push(`Direção de estilo: ${promptOptions.styleDirection}.`);
  if (promptOptions.preserveDetails) sections.push(`Preservar obrigatoriamente: ${promptOptions.preserveDetails}.`);
  if (promptOptions.extraInstructions) sections.push(`Instruções extras: ${promptOptions.extraInstructions}.`);
...
function buildPromptAutocompleteOptions(query) {
  if (!query) return [];
  if (query.marker === "@") {
    return state.productModels.filter((e) => !query.search || e.alias.toLowerCase().startsWith(query.search) || e.name.toLowerCase().includes(query.search)).slice(0, 6).map((e) => ({ marker: "@", alias: e.alias, name: e.name, meta: e.notes || "Modelo de produto" }));
  }
  if (query.marker === "#") {
    return state.imageTemplates.filter((e) => !query.search || e.alias.toLowerCase().startsWith(query.search) || e.name.toLowerCase().includes(query.search)).slice(0, 6).map((e) => ({ marker: "#", alias: e.alias, name: e.name, meta: e.notes || deps.buildPromptDetailsSummary(e.promptOptions) || "Template visual" }));
  }
  return [];
}
...
export function updatePromptAutocomplete() {
  ...
  if (!query) { hidePromptAutocomplete(); return; }
  const options = buildPromptAutocompleteOptions(query);
  if (!options.length || !promptAutocomplete || !promptAutocompleteList) { hidePromptAutocomplete(); return; }
  promptAutocompleteState = { query, options, activeIndex: Math.min(promptAutocompleteState.activeIndex, options.length - 1) };
  renderPromptAutocomplete();
}

function renderPromptAutocomplete() {
  if (!promptAutocomplete || !promptAutocompleteList || !promptAutocompleteState.options.length) { hidePromptAutocomplete(); return; }
  promptAutocomplete.hidden = false;
  promptAutocompleteList.innerHTML = promptAutocompleteState.options.map((option, index) => `
  ...
  for (const button of promptAutocompleteList.querySelectorAll("[data-autocomplete-index]")) {
    button.onmousedown = (event) => { event.preventDefault(); const index = Number(button.getAttribute("data-autocomplete-index")); if (Number.isFinite(index)) applyPromptAutocompleteOption(index); };
  }
}
...
export function handlePromptAutocompleteKeydown(event) {
  ...
  if (event.key === "ArrowDown") { event.preventDefault(); promptAutocompleteState.activeIndex = (promptAutocompleteState.activeIndex + 1) % promptAutocompleteState.options.length; renderPromptAutocomplete(); return; }
  if (event.key === "ArrowUp") { event.preventDefault(); promptAutocompleteState.activeIndex = (promptAutocompleteState.activeIndex - 1 + promptAutocompleteState.options.length) % promptAutocompleteState.options.length; renderPromptAutocomplete(); return; }
  if (event.key === "Enter" || event.key === "Tab") { event.preventDefault(); applyPromptAutocompleteOption(promptAutocompleteState.activeIndex); return; }
...
function applyPromptAutocompleteOption(index) {
  ...
  if (!option || !query || !promptInput) { hidePromptAutocomplete(); return; }
...
export function persistCustomPromptPresets() {
  persistCustomPromptPresetsToStorage(window.localStorage, state.customPromptPresets, CUSTOM_PRESETS_STORAGE_KEY);
}

export function renderCustomPromptPresets() {
  if (!customPresetList) return;
  if (!state.customPromptPresets.length) { customPresetList.innerHTML = `<p class="custom-preset-empty">Nenhum preset salvo ainda.</p>`; bindCustomPresetActions(); return; }
  customPresetList.innerHTML = state.customPromptPresets.map((preset) => `
...
function bindCustomPresetActions() {
  for (const button of document.querySelectorAll("[data-apply-custom-preset]")) {
    button.onclick = () => { const preset = state.customPromptPresets.find((e) => e.id === button.getAttribute("data-apply-custom-preset")); if (!preset) return; hydratePromptOptions(preset.options); statusBox.textContent = `Preset "${preset.name}" aplicado.`; };
  }
  for (const button of document.querySelectorAll("[data-delete-custom-preset]")) {
    button.onclick = async () => {
      ...
      if (!preset) return;
      const confirmed = await deps.requestConfirmation({ title: "Excluir preset", message: `Excluir o preset "${preset.name}"?`, confirmLabel: "Excluir" });
      if (!confirmed) return;
...
export function saveCurrentPromptPreset() {
  ...
  if (!name) { statusBox.textContent = "De um nome ao preset antes de salvar."; customPresetNameInput?.focus(); return; }
  const options = sanitizePromptOptions(collectPromptOptions());
  const existingIndex = state.customPromptPresets.findIndex((e) => e.name.toLowerCase() === name.toLowerCase());
  const nextPreset = { id: existingIndex >= 0 ?state.customPromptPresets[existingIndex].id : `preset-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, name, options };
  if (existingIndex >= 0) { state.customPromptPresets.splice(existingIndex, 1, nextPreset); statusBox.textContent = `Preset "${name}" atualizado.`; }
  else { state.customPromptPresets.unshift(nextPreset); state.customPromptPresets = state.customPromptPresets.slice(0, 12); statusBox.textContent = `Preset "${name}" salvo.`; }
  persistCustomPromptPresets();
  renderCustomPromptPresets();
