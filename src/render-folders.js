...
import { galleryFolderFilter, cutoutFolderFilter, cropFolderFilter, folderFilterInput, folderBoard, folderSummary, folderDialogOptions } from "./dom.js";

export function getExistingFolderNames() {
  return Array.from(new Set([
    ...state.customFolders,
    ...state.lastJobs.filter((j) => j.status === "completed" && j.result?.folder).map((j) => String(j.result.folder || "").trim()),
...
export function populateSectionFolderFilters() {
  ...
  for (const select of [galleryFolderFilter, cutoutFolderFilter, cropFolderFilter]) {
    ...
    const hasCurrentValue = currentValue === "all" || currentValue === "" || folderNames.includes(currentValue);
...
export function renderFolderDialogOptions(currentFolder = "") {
  ...
  if (!folders.length) { folderDialogOptions.innerHTML = `<p class="empty-state">Nenhuma pasta criada ainda.</p>`; return; }
  folderDialogOptions.innerHTML = folders.map((folder) => `
    <button class="folder-choice${folder === currentFolder ?" is-active" : ""}" type="button" data-folder-choice="${escapeHtml(folder)}">${escapeHtml(folder)}</button>`).join("");
}
...
export function renderFolderGroupedCollection(container, items, cardFactory, emptyMessage) {
  ...
  if (!groups.length) { container.innerHTML = `<p class="empty-state">${emptyMessage}</p>`; return; }
  container.innerHTML = "";
...
export function renderFolderBoard() {
  ...
  const entries = [
    ...state.lastJobs.filter((j) => j.status === "completed" && j.result).map((j) => ({ folder: j.result.folder || "", kind: "Galeria" })),
  ...
  const grouped = new Map();
  for (const folder of getExistingFolderNames()) grouped.set(folder, { total: 0, gallery: 0, cutouts: 0, crops: 0 });
  ...
  const visibleFolders = Array.from(grouped.entries()).filter(([name]) => !folderQuery || name.toLowerCase().includes(folderQuery)).sort((a, b) => {
    if (a[0] === "Sem pasta") return -1;
    if (b[0] === "Sem pasta") return 1;
    return a[0].localeCompare(b[0], "pt-BR");
  });
  folderSummary.textContent = `${visibleFolders.length} pasta(s) visiveis`;
  if (!visibleFolders.length) { folderBoard.innerHTML = `<p class="empty-state">Nenhuma pasta corresponde ao filtro atual.</p>`; return; }
  folderBoard.innerHTML = visibleFolders.map(([folderName, counts]) => `
