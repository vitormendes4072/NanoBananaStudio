...
export function requestComposerPanelPinning() {
  ...
  composerPinFrame = window.requestAnimationFrame(() => { composerPinFrame = 0; syncComposerPanelPinning(); });
}

function syncComposerPanelPinning() {
  if (!appLayout || !composerColumn || !composerPanel) return;
  if (composerExpanded) { resetComposerPanelPinning(); return; }
  const desktopLayout = window.innerWidth > 900;
  if (!desktopLayout) { resetComposerPanelPinning(); return; }
  const topOffset = 8;
  ...
  if (currentScroll <= fixedStart) { resetComposerPanelPinning(true); return; }
  if (currentScroll >= fixedEnd) {
    composerPanel.style.position = "absolute"; composerPanel.style.top = "auto"; composerPanel.style.right = "auto";
    composerPanel.style.bottom = "0"; composerPanel.style.left = "0"; composerPanel.style.width = "100%"; return;
  }
  composerPanel.style.position = "fixed"; composerPanel.style.top = `${topOffset}px`; composerPanel.style.right = "auto";
  composerPanel.style.bottom = "auto"; composerPanel.style.left = `${columnRect.left}px`; composerPanel.style.width = `${columnRect.width}px`;
}
...
function resetComposerPanelPinning(keepColumnHeight = false) {
  ...
  composerPanel.style.position = "relative"; composerPanel.style.top = ""; composerPanel.style.right = "";
  composerPanel.style.bottom = ""; composerPanel.style.left = ""; composerPanel.style.width = "";
  composerPanel.style.maxHeight = ""; composerPanel.style.overflowY = "";
}
...
export function collapseComposer() {
  if (composerExpanded) { composerExpanded = false; syncComposerExpandedState(); }
}

export function isComposerExpanded() { return composerExpanded; }
