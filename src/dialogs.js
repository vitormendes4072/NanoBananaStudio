...
import {
  confirmDialog, confirmDialogTitle, confirmDialogMessage, confirmDialogCancelButton, confirmDialogConfirmButton,
  folderDialog, folderDialogTitle, folderDialogMessage, folderDialogCurrent, folderDialogInput,
  folderDialogOptions, folderDialogClearButton, folderDialogCancelButton, folderDialogConfirmButton,
} from "./dom.js";

let confirmDialogResolver = null;
let folderDialogResolver = null;

export function requestConfirmation({ title = "Confirmar", message = "Deseja continuar?", confirmLabel = "Confirmar" } = {}) {
  if (!confirmDialog || !confirmDialogTitle || !confirmDialogMessage || !confirmDialogConfirmButton || !confirmDialogCancelButton) {
    return Promise.resolve(window.confirm(message));
  }
  if (confirmDialog.open && confirmDialogResolver) { confirmDialogResolver(false); confirmDialogResolver = null; }
  ...
  return new Promise((resolve) => {
    ...
    const handleCancel = (event) => { event.preventDefault(); cleanup(false); };
...
export function requestFolderSelection({ title = "Organizar em pasta", message = "Escolha uma pasta existente ou crie uma nova.", currentFolder = "" } = {}) {
  if (!folderDialog || !folderDialogTitle || !folderDialogMessage || !folderDialogCurrent || !folderDialogInput || !folderDialogOptions || !folderDialogClearButton || !folderDialogCancelButton || !folderDialogConfirmButton) {
    return Promise.resolve(window.prompt(message, currentFolder));
  }
  if (folderDialog.open && folderDialogResolver) { folderDialogResolver(null); folderDialogResolver = null; }
  ...
  return new Promise((resolve) => {
    ...
    const cleanup = (result) => {
      ...
      for (const button of folderDialogOptions.querySelectorAll("[data-folder-choice]")) button.removeEventListener("click", handleChoiceClick);
    ...
    const handleChoiceClick = (event) => { cleanup(event.currentTarget.getAttribute("data-folder-choice") || ""); };
    const handleCancel = (event) => { event.preventDefault(); cleanup(null); };
    ...
    for (const button of folderDialogOptions.querySelectorAll("[data-folder-choice]")) button.addEventListener("click", handleChoiceClick);
