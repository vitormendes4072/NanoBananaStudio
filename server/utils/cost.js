...
import { normalizeProductModelEvaluation, parseJsonObject } from './serialization.js';
...
export function buildHeuristicProductModelEvaluation(productModel) {
  const referenceCount = Array.isArray(productModel.referenceImages)
  ...
  if (referenceCount >= 3) {
    strengths.push(
      'Conjunto com variedade inicial de referências para reduzir variação na geração.'
    );
  ...
  if (referenceCount < 3) {
    missing.push(
      'Ainda não há cobertura suficiente para reproduções mais fiéis em cenas variadas.'
    );
  ...
  if (mode === 'gemini') {
    if (!apiKey) {
      throw badRequestError(
        'A avaliacao com IA precisa de GEMINI_API_KEY configurada no servidor.'
      );
    }
    evaluation =
