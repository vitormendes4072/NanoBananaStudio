...
import {
  serializeJob, normalizeQuantity, normalizePromptOptions, normalizeReferenceImages,
  normalizeBranchReference, normalizeIdList, normalizeLibraryFolder,
  resolveProductModelsByAlias, resolveImageTemplatesByAlias,
  buildBatchId, pickAllowedValue, storeReferenceImages,
  buildJobProductModelMeta, buildJobImageTemplateMeta,
} from "../utils.js";
...
router.post("/api/jobs", generationLimiter, (req, res) => {
  ...
  if (!apiKey) {
    return res.status(400).json({ error: "Configure a GEMINI_API_KEY no arquivo .env antes de gerar imagens." });
  }
  ...
  const model = pickAllowedValue(body.model, ["gemini-2.5-flash-image", "gemini-3-pro-image-preview"], "gemini-2.5-flash-image");
  const batchId = quantity > 1 ? buildBatchId() : null;

  const createdJobs = [];
  for (let index = 0; index < quantity; index++) {
    createdJobs.push(createJob({
      prompt, promptBase, promptOptions, model,
...
router.post("/api/jobs/:id/cancel", (req, res) => {
  ...
  if (job.status !== "queued") {
    return res.status(409).json({ error: "Apenas jobs na fila podem ser cancelados.", job: serializeJob(job) });
  }
