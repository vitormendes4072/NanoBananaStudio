...
import {
  upsertProductModel, upsertImageTemplate,
  deleteProductModel, deleteImageTemplate, assignLibraryFolder,
} from "../media.js";
import { deleteLibraryBulk } from "../queue.js";
import { libraryLimiter } from "../rateLimits.js";
import {
  serializeProductModel, serializeImageTemplate,
  normalizeIdList, normalizeLibraryFolder, evaluateProductModelQuality,
} from "../utils.js";
...
router.post("/api/product-models/:alias/evaluate", libraryLimiter, async (req, res) => {
  ...
  res.json({ ok: true, productModel: serializeProductModel(productModel), evaluation: productModel.evaluation });
});
